...
@section('content-header')
    <h1>
        {{ trans('carrental::reservations.title.create reservation') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.carrental.reservation.index') }}">{{ trans('carrental::reservations.title.reservations') }}</a></li>
        <li class="active">{{ trans('carrental::reservations.title.create reservation') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.carrental.reservation.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('carrental::admin.reservations.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
